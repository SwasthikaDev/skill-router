...
- `reliability` (bool, optional, default false): if true, Skill-Router cross-checks each
  result against **CipherWatch** (the signed liveness/uptime oracle for the agent web) and
  adds a `cipherwatch` field per result — `{ "live": true/false, "uptime_pct": 98 }` —
...
- `only_live` (bool, optional, default false): with `reliability` true, drop results
  CipherWatch reports as not live, so you only get callable agents.
